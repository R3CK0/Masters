...
class GameStateEditor:
    def __init__(self, root):
        ...
        ttk.Button(self.frame, text="Save", command=self.save_configuration).grid(column=3, row=0)
    ...
    def on_combobox_select(self, event, combo, allow_create=True):
        ...
        if combo_name == "Combine" and allow_create:
            ...
            state_combo.bind("<<ComboboxSelected>>", lambda event, state_combo=state_combo, main_object=allow_create: self.state_change_select(event, state_combo, main_object))
            #self.root.wait_window(self.temporary_window)
            #self.temporary_window = None
        if self.effects["effect1"].get() != "Combine" and self.effects["effect2"].get() != "Combine":
            self.input_area_3.destroy()
            if self.effects["effect1"].get() != "Create" and self.effects["effect2"].get() != "Create":
                self.input_area_2.destroy()
    ...
    def state_change_select(self, event, state_combo, main_object):
        ...
        if self.states[state_name]["type"] == "Boolean":
            ...
            new_state.grid(row=1, column=1)
        new_state.bind("<<ComboboxSelected>>", lambda event, new_state=new_state, state_name=state_name, main_object=main_object: self.state_change_select_update(event, self.states[state_name]["type"], new_state.get(), main_object, state_name))
    ...
    def state_change_select_update(self, event, state_type, change_option, main_object, state_name):
        ...
        if state_type == "Boolean":
            if change_option == "Toggle":
                ...
                self.state_change[position] = f"Set {state_name} to {new_value.get()}"
        elif state_type == "Discrete":
            if change_option == "Set":
                ttk.Label(self.temporary_window, text="New State:").grid(row=2, column=0)
                new_state = ttk.Combobox(self.temporary_window, values=self.states[state_name]["states"])
                new_state.grid(row=2, column=1)
                self.state_change[position] = f"Set {state_name} to {new_state.get()}"
            else:
            ...
            if change_option == "Increase":
                ...
                new_value.grid(row=2, column=1)
                self.state_change[position] = f"Increase {state_name} by {new_value.get()}"
            elif change_option == "Decrease":
                ttk.Label(self.temporary_window, text="Decrease by:").grid(row=2, column=0)
                new_value = ttk.Entry(self.temporary_window)
                new_value.grid(row=2, column=1)
                self.state_change[position] = f"Decrease {state_name} by {new_value.get()}"
            else:
                ttk.Label(self.temporary_window, text="New Value:").grid(row=2, column=0)
                new_value = ttk.Entry(self.temporary_window)
                new_value.grid(row=2, column=1)
        ...
        if self.entries["name"].get() and self.location_combos["location"].get():
            ...
            for effect in [self.effects["effect1"].get(), self.effects["effect2"].get()]:
                if effect == "Create":
                    ...
                    effects.append(f"Create {self.entries['name_create'].get()} at location {self.location_combos['location_create'].get()} is movable = {self.movable['movable_create'].get()} is consumed = {self.consumed['consumed_create'].get()}, effects = {self.effects['effect1_create'].get()}, {self.effects['effect2_create'].get()}")
                elif effect == "Combine":
                    if self.effects["effect1_create"].get() == "Change State":
                        self.effects["effect1_create"] = self.state_change["secondary"]
                    elif self.effects["effect2_create"].get() == "Change State":
                        self.effects["effect2_create"] = self.state_change["secondary"]
                    effects.append(f"Combine {self.entries['name'].get()} with {self.entries['name_combo'].get()} create {self.entries['name_create'].get()} is movable = {self.movable['movable_create'].get()} is consumed = {self.consumed['consumed_create'].get()}, effects = {self.effects['effect1_create'].get()}, {self.effects['effect2_create'].get()}")
                    self.objects[self.entries['name_combo'].get()] = {
                        ...
                        "effects": [f"Combine {self.entries['name'].get()} with {self.entries['name_combo'].get()} create {self.entries['name_create'].get()} is movable = {self.movable['movable_create'].get()} is consumed = {self.consumed['consumed_create'].get()}, effects = {self.effects['effect1_create'].get()}, {self.effects['effect2_create'].get()}"
                    , "None"]
                    }
    ...
    def  add_state(self, state_type):
        if self.name:
            ...
            # Set, toggle, increase, decrease, rotate
