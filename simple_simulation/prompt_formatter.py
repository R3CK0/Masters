...
class PromptFormatter:
    ...
    def process_environement(self) -> None:
        ...
        for state in list(states.values()):
            ...
            if state_type == "Boolean":
                ...
                state_value = None
                values = state["states"]
                new_state = State(state_name, state_value, state_type, values=values)
        ...
        for object_name, object in objects.items():
            ...
            if object["effects"] == []:
                ...
                effects = [effect[0].split(" ")[0] if effect[0].split(" ")[0] in ["Create", "Combine", "Set", "Rotate", "Increase", "Decrease", "Delete", "Toggle", "Unset"] else None for effect in object["effects"]]
                create_object = [None for _ in range(len(effects))]
                state = [None for _ in range(len(effects))]
                value = [None for _ in range(len(effects))]
                amount = [None for _ in range(len(effects))]
                delete = [None for _ in range(len(effects))]
                toggle = [None for _ in range(len(effects))]
                if effects is not None:
                    for idx, effect in enumerate(effects):
                        if effect is not None:
                            tokens = object["effects"][idx][0].split(" ")
                            if effect == "Create":
                                create_object[idx] = " ".join(tokens[1: tokens.index("at")])
                                self.add_effect_object(object["effects"][idx])
                            elif effect == "Set" or effect == "Unset":
                                if "Predicate" in tokens:
                                    state[idx] = " ".join(tokens[1: tokens.index("Predicate")])
                                    value[idx] = " ".join(tokens[tokens.index("to")+1:])
                                else:
            ...
            new_object = Object(object_name, movable, consumed, requires_states, requires_objects, required_location, effects, create_object, delete, state, value, toggle, amount)
    ...
    def add_effect_object(self, object) -> None:
        ...
        list_effects = " ".join(tokens[tokens.index("effects")+2:tokens.index("requires_objects")]).split(",")
        ...
        if list_effects[0] == "[]":
            ...
            effects = [effect.split(" ")[0] if effect.split(" ")[0] in ["Create", "Combine", "Set", "Rotate", "Increase", "Decrease", "Delete", "Toggle", " Unset"] else None for effect in list_effects]
            create_object = [None for _ in range(len(effects))]
            state = [None for _ in range(len(effects))]
            value = [None for _ in range(len(effects))]
            amount = [None for _ in range(len(effects))]
        ...
        self.temporaty_objects[object_name] = Object(object_name, movable, consumed, required_states, required_objects, required_location, effects, create_object, delete, state, value, toggle, amount)        
    ...
    def generate_environemental_prompt(self, state_list, tool_list):
        ...
        for state in state_list:
            if self.environement.states[state].value_type == "Predicate":
                prompt += f"- Predicate: {state} , variables are {self.environement.states[state].values}\n"
            else:
                prompt += f"- {state}\n"
        prompt += f"Tools:\n"
    ...
    def format_effects(self, tool_list):
        ...
        for object in tool_list:
            if self.environement.objects[object].effects != None:    
                ...
                effect_types = self.environement.objects[object].effects
                ...
                for idx, effect_type in enumerate(effect_types):
                    ...
                    elif effect_type == "Unset":
                        prompt += f"sets the {self.environement.objects[object].state[idx]} predicate to not {self.environement.objects[object].state[idx]}\n"
